...
* 扫描当前目录：`python threadviper.py`
* 扫描指定文件或目录：`python threadviper.py 测试用例1.txt 测试用例2.txt`
...
* **目标 3：死锁与野线程捕获。**
* 在 `in_critical_section == True` 的期间，如果扫描到了 `return`、`break` 或 `exit(0)`，直接爆出**严重高危漏洞 (Critical)**：**“检测到临界区未释放即退出，可能导致全局死锁！”** 。


* 全局搜索 `TerminateThread` 。这是一个在 C++ 中极其危险的强杀 API，会导致目标线程的栈内存和锁彻底泄漏。直接将其标记为**架构级风险**，并建议改用事件通知 (Event) 让线程优雅退出。
